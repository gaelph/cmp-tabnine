...
function Source.complete(self, ctx, callback)
  ...
  self.pending[self.job] = { ctx = ctx, callback = callback }
...
function Source.on_stdout(self, job, data)
  -- {
  --   "old_prefix": "wo",
  --   "results": [
  --     {
  ...
  for _, jd in ipairs(data) do
    if jd ~= nil and jd ~= '' and jd ~= 'null' then
      ...
      elseif self.pending[job] == nil then
        dump('TabNine: unknown message: ', jd)
      else
        local ctx = self.pending[job].ctx
        local callback = self.pending[job].callback
